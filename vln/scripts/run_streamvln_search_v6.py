#!/usr/bin/env python3
"""Twelve StreamVLN val_unseen candidates, then frozen val_seen evaluation.

Each GPU owns one serial worker and a process-lifetime lock. Tent and FSTTA use frozen
historical val_unseen choices; neither is searched again. Incomplete
...
SPEC = REPO_ROOT / "vln/experiments/streamvln_val_unseen_search_v6.json"
RESULTS = REPO_ROOT / "vln/results/tuning/streamvln_val_unseen_search_v6"
...
METHODS = ("eam", "feedtta", "atena")
FROZEN_METHODS = ("tent", "fstta")
...
def assert_committed_runtime():
    ...
    paths = ["core", "vln/baselines/streamvln", "vln/navtta_vln", "vln/scripts",
             ...
             "vln/experiments/streamvln_val_unseen_search_v6.json"]
...
def load_spec():
    spec = read_json(SPEC)
    if spec.get("schema") != "navtta.streamvln_search.v6":
        raise ValueError("invalid v6 search spec")
    ...
    if spec.get("selection") != {"primary_metric": "SPL", "secondary_metric": "SR"}:
        raise ValueError("v6 selection is fixed to SPL, then SR")
    if set(spec["methods"]) != set(METHODS):
        raise ValueError("only EAM/FeedTTA/ATENA may be searched")
    ...
    if sha256(audit_path) != audit["sha256"]:
        raise ValueError("historical parameter-selection audit changed")
    historical = read_json(audit_path)["frozen_selections"]
    if set(spec["frozen_selections"]) != set(FROZEN_METHODS):
        raise ValueError("Tent and FSTTA must both be frozen historical selections")
    for method in FROZEN_METHODS:
        if spec["frozen_selections"][method] != historical[method]:
            raise ValueError("frozen selection differs from audited val_unseen winner: " + method)
    if spec["frozen_selections"]["tent"]["parameters"].get("streamvln_readout_protocol") != "legacy_v4":
        raise ValueError("Tent must retain its selected v4 protocol")
    if spec["frozen_selections"]["fstta"]["parameters"].get("streamvln_readout_protocol") != "native_residual":
        raise ValueError("FSTTA must retain its selected v5 protocol")
    return spec
...
def make_job(method, split, candidate_id, parameters, selection=None, episodes=None):
    is_smoke = episodes is not None
    episodes = EXPECTED[split] if episodes is None else episodes
    short_split = "vu" if split == "val_unseen" else "vs"
    if is_smoke:
        short_split = "smoke"
    tag = "streamvln-{}-{}-{}-v6".format(short_split, method, candidate_id)
    ...
    document = {
        ...
        "setting": SETTING, "split": split, "episodes": episodes, "seed": 0,
        "episode_order_manifest_sha256": sha256(ORDER_ROOT / (split + ".json")),
    ...
    if is_smoke:
        output = REPO_ROOT / "vln/results/smoke" / tag / SETTING / split
    translate(SETTING, config, output / "tta_diagnostics.json")
    return Job(tag, method, split, config, output, episodes, is_smoke)
...
def smoke_jobs(spec):
    parameters = {"source": {}}
    parameters.update({m: spec["frozen_selections"][m]["parameters"] for m in FROZEN_METHODS})
    parameters.update({m: spec["methods"][m][0]["parameters"] for m in METHODS})
    return [make_job(method, "val_seen", "preflight", point, episodes=2)
            for method, point in parameters.items()]
...
def bind_historical_selections(spec):
    # Raw v4/v5 archives remain legacy. Only their audited winning parameters
    # are imported; their metrics never become completed v6 evaluations.
    document = {"audit": spec["historical_audit"],
                "selection_split": "val_unseen", "records": spec["frozen_selections"]}
    write_once(RESULTS / "historical_selection.json", document)
    return document
...
def verify_assets(splits):
    ...
    path = REPO_ROOT / "vln/manifests/generated" / ("streamvln-v6-assets-" + fingerprint[:16] + ".json")
...
def exclusive_lease(path, description):
    ...
    with path.open("a+") as stream:
        try:
            ...
            raise RuntimeError(description + " is reserved by another StreamVLN v6 worker")
        try:
            yield stream.fileno()
        finally:
...
def run_one_job(gpu, job, asset_record, gpu_lock, retry_incomplete):
    ...
    with job_lease(job) as job_lock:
        ...
        if completed(job):
            ...
            return
        # Also catches pre-v6 processes, which do not acquire our locks.
        assert_gpu_idle(gpu)
...
def select_winners(jobs):
    if len(jobs) != 12 or any(not completed(job) for job in jobs):
        raise ValueError("selection requires all 12 authenticated, complete val_unseen results")
    if any(len({getattr(job, field) for job in jobs}) != 12 for field in ("tag", "config", "output")):
        raise ValueError("selection requires 12 distinct jobs, configs, and output directories")
    records = {}
    for method in METHODS:
    ...
    selection = {"schema": "navtta.streamvln_selection.v6", "selection_split": "val_unseen",
...
def seen_jobs(spec, selection):
    ...
    jobs += [make_job(method, "val_seen", "frozen", spec["frozen_selections"][method]["parameters"],
                     {"historical_run_tag": spec["frozen_selections"][method]["run_tag"],
                      "historical_audit": spec["historical_audit"], "selection_split": "val_unseen"})
             for method in FROZEN_METHODS]
...
def report(jobs):
    ...
    for job in jobs:
        ...
        if row["complete"]:
            ...
            if (job.split, job.episodes) in baselines:
                row["delta_vs_source_pp"] = {key: row[key] - baselines[(job.split, job.episodes)][key] for key in ("SR", "SPL")}
            if job.method != "source":
                diag = read_json(job.output / "tta_diagnostics.json")
    ...
    atomic_json(RESULTS / "report.json", {
        "records": records, "historical_parameter_selections": legacy,
    ...
    lines = ["# StreamVLN v6", "", "val_unseen selects parameters; val_seen evaluates frozen choices.", "",
             "| Run | Split | Episodes | SR | SPL | ΔSR | ΔSPL |", "|---|---|---:|---:|---:|---:|---:|"]
    if legacy:
        lines.insert(4, "Tent/FSTTA use audited historical val_unseen parameters. Historical metrics are not v6 results.\n")
    for row in records:
        ...
        if row["complete"]:
            ...
            lines.append("| {} | {} | {} | {:.4f} | {:.4f} | {} | {} |".format(
                row["run_tag"], row["split"], row["episodes"], row["SR"], row["SPL"],
                "{:+.4f}".format(delta["SR"]) if delta else "—",
                "{:+.4f}".format(delta["SPL"]) if delta else "—"))
        else:
            lines.append("| {} | {} | {} | incomplete | — | — | — |".format(row["run_tag"], row["split"], row["episodes"]))
...
def main():
    ...
    parser.add_argument("--stage", choices=("smoke", "search", "seen", "all", "report"), default="search")
    ...
    try:
        ...
        if args.dry_run:
            ...
            if args.stage != "smoke":
                print("After 12 complete val_unseen runs: freeze winners by SPL, then SR; val_seen never selects parameters.")
            if args.stage in ("seen", "all"):
                if all(completed(job) for job in jobs):
                    ...
                    print("val_seen: Source + frozen Tent/FSTTA + three search winners (6 jobs; winners pending).")
            return 0
        assert_committed_runtime()
        bind_historical_selections(spec)
        ...
        if args.stage == "seen":
            selection = select_winners(jobs)
        splits = ["val_unseen", "val_seen"] if args.stage == "all" else ["val_seen" if args.stage in ("seen", "smoke") else "val_unseen"]
