...
METHOD="${2:?missing method; AV-Nav currently supports source evaluation here}"
SEED="${3:?missing seed}"
shift 3
...
if [[ "${METHOD}" != "source" ]]; then
    printf 'eval_av_nav.sh supports source only; got: %s\n' "${METHOD}" >&2
    exit 2
fi
...
CONFIG="ss_baselines/av_nav/config/tta_avn/${SOURCE_SETTING}/test.yaml"
CHECKPOINT="${REPO_ROOT}/avn/checkpoints/source/av_nav/${CHECKPOINT_NAME}"
...
RUN_ID="avn-mp3d-av_nav-source-${SOURCE_SETTING}-seed${SEED}${RUN_TAG_SUFFIX}-$(date -u +%Y%m%dT%H%M%SZ)-${COMMIT}"
...
test -f "${BASELINE_ROOT}/${CONFIG}" || {
    printf 'missing AV-Nav evaluation config: %s\n' "${BASELINE_ROOT}/${CONFIG}" >&2
    exit 1
}
test -f "${DATASET}" || {
    printf 'missing TTA evaluation dataset: %s\n' "${DATASET}" >&2
    exit 1
}
...
python3 "${REPO_ROOT}/tools/create_run_manifest.py" \
    ...
    --model av_nav --method source --run-tag "${RUN_TAG}" \
...
set +e
python3 ss_baselines/av_nav/run.py \
    --run-type eval \
    --exp-config "${CONFIG}" \
