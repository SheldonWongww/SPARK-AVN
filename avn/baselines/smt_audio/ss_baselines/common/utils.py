...
class CategoricalNetWithMask(nn.Module):
    ...
    def forward(self, features, action_maps):
        # NOTE: dim=-1 is critical. Without it PyTorch falls back to softmax
        # over the entire flattened tensor, which makes every action's
        # probability ~1/(B*A) and the policy's entropy collapses to ~0
        # immediately. This is a long-standing bug in the original repo.
        probs = f.softmax(self.linear(features), dim=-1)
        if self.masking:
            probs = probs * torch.reshape(action_maps, (action_maps.shape[0], -1)).float()
            # re-normalize after masking so probs sum to 1 along action dim;
            # otherwise the masked-out positions skew log_prob and entropy.
