...
class BeliefPredictor(nn.Module):
    def __init__(self, belief_config, device, input_size, pose_indices,
        ...
        # Load a frozen, author-pretrained location predictor (goal descriptor)
        # instead of training it online with ground-truth pointgoal coordinates.
        self.load_descriptor = getattr(belief_config, 'load_pretrained_descriptor', False)
        # Whether cnn_forward should concatenate distractor category labels to
        # the spectrogram (only the 23-channel online predictor expects this).
        self.concat_distractor_labels = False

        if self.predict_location:
            if belief_config.online_training or self.load_descriptor:
                # custom_resnet18 architecture, matching the author's released
                # SAVi location predictor (predictor.* in best_val.pth).
                if self.has_distractor_sound and belief_config.online_training:
                    self.predictor = custom_resnet18(num_input_channels=23)
                    self.concat_distractor_labels = True
                else:
    ...
    def load_pretrained_weights(self):
        if self.predict_location:
            if self.load_descriptor:
                # Load the author-pretrained location predictor (goal descriptor)
                # and keep it frozen. This avoids using ground-truth pointgoal
                # coordinates as online supervision during policy training.
                state_dict = torch.load(self.config.descriptor_weights, map_location='cpu')
                source = state_dict['belief_predictor'] if 'belief_predictor' in state_dict else state_dict
                cleaned_state_dict = {
                    k[len('predictor.'):]: v for k, v in source.items()
                    if k.startswith('predictor.')
                }
                self.predictor.load_state_dict(cleaned_state_dict)
                logging.info("Loaded pretrained (frozen) location predictor / goal descriptor")
            else:
    ...
    def cnn_forward(self, observations):
        ...
        if self.concat_distractor_labels:
