...
def find_best_ckpt_idx(event_dir_path, min_step=-1, max_step=10000):
    # Import tensorflow lazily here. The top-level import of tf was removed
    # because it competes with torch for the CUDA context during sim worker
    # startup (causes SIGSEGV). Only this function actually needs tf to read
    # tensorboard event files, so we import it on-demand.
