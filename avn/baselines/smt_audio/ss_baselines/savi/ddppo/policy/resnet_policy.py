...
class ResNetEncoder(nn.Module):
    ...
    def forward(self, observations):
        ...
        if self._n_input_rgb > 0:
            ...
            # Defensive: collapse unexpected singleton dims so shape becomes 4D [B, H, W, C]
            while rgb_observations.dim() > 4:
                if rgb_observations.size(-1) == 1:
                    rgb_observations = rgb_observations.squeeze(-1)
                elif rgb_observations.size(0) == 1:
                    rgb_observations = rgb_observations.squeeze(0)
                else:
                    break
        ...
        if self._n_input_depth > 0:
            ...
            # Defensive: collapse unexpected singleton dims so shape becomes 4D [B, H, W, C]
            while depth_observations.dim() > 4:
                if depth_observations.size(-1) == 1:
                    depth_observations = depth_observations.squeeze(-1)
                elif depth_observations.size(0) == 1:
                    depth_observations = depth_observations.squeeze(0)
                else:
                    break
