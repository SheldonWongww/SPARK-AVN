...
# 这里做了修改，避免保存在用户主目录下，因为对应的硬盘存储不够了
# INTERRUPTED_STATE_FILE = osp.join(
#     "${HOME}", ".interrupted_states", f"{SLURM_JOBID}.pth"
# )
INTERRUPTED_STATE_FILE = osp.join(
    "/data1/wxy", ".interrupted_states", f"{SLURM_JOBID}.pth"
)
...
def save_interrupted_state(state: Any, filename: str = None):
    ...
    :param filename: The filename.  Defaults to "/data1/wxy/.interrupted_states/${SLURM_JOBID}.pth"
    """
...
def load_interrupted_state(filename: str = None) -> Optional[Any]:
    ...
    :param filename: The filename of the saved state.
        Defaults to "/data1/wxy/.interrupted_states/${SLURM_JOBID}.pth"
...
def init_distrib_slurm(
    ...
    # Use env:// init method for process group (compatible with PyTorch 2.x
    # torch.distributed.launch which binds to MASTER_PORT internally)
    ...
    # Create a separate TCPStore on a different port for rollout tracking
    tcp_store = distrib.TCPStore(
        master_addr, master_port + 1, world_size, world_rank == 0
    )
