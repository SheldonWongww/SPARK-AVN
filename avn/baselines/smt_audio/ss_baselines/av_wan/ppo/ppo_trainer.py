...
class PPOTrainer(BaseRLTrainer):
    ...
    def _extract_scalars_from_info(cls, info: Dict[str, Any]) -> Dict[str, float]:
        ...
        for k, v in info.items():
            # skip non-scalar / visualization-only entries
            if k in {'top_down_map', 'collisions', 'rgb_frames', 'audios'}:
                continue
            ...
            elif isinstance(v, (int, float, np.integer, np.floating, bool, np.bool_)):
                result[k] = float(v)
        return result
    ...
    def _collect_rollout_step(
        ...
        # accumulate every scalar navigation metric returned by the env
        # (success, spl, softspl, na, sna, sws, distance_to_goal, ...)
        for k, v in self._extract_scalars_from_infos(infos).items():
            v = torch.tensor(v, dtype=torch.float).unsqueeze(1)
    ...
    def train(self) -> None:
        ...
        # Force CUDA initialization in the main process BEFORE construct_envs
        # forks worker subprocesses. CUDA context cannot be lazily initialized
        # after fork, otherwise torch.cuda._lazy_init segfaults when we later
        # call actor_critic.to(self.device).
        ...
        # episode stats accumulate over the entire training course; running_episode_stats
        # holds every scalar metric returned by the env (success, spl, softspl, na, ...)
        ...
        with TensorboardWriter(
            ...
            for update in range(start_update, self.config.NUM_UPDATES):
                ...
                num_window_episodes = deltas["count"]
                deltas["count"] = max(deltas["count"], 1.0)

                # reward + every navigation metric collected from the env info
                # (success, spl, softspl, na, sna, sws, distance_to_goal, ...)
                writer.add_scalar(
                ...
                for metric, value in deltas.items():
                    if metric in {"reward", "count"}:
                        continue
                    writer.add_scalar(f"Environment/{metric}", value / deltas["count"], count_steps)
                ...
                if update > 0 and update % self.config.LOG_INTERVAL == 0:
                    ...
                    if num_window_episodes > 0:
                        window_size = len(window_episode_stats["count"])
                        metrics_str = "  ".join(
                        ...
                        logger.info(
                            "[window={}] {}".format(window_size, metrics_str)
                        )
                        logger.info(
                            "[losses]    value_loss={:.4f}  action_loss={:.4f}  entropy={:.4f}  lr={:.2e}".format(
                                value_loss, action_loss, dist_entropy, lr_scheduler.get_lr()[0]
                            )
                        )
                    else:
