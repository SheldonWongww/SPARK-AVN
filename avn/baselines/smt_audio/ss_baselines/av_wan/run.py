...
def find_best_ckpt_idx(event_dir_path, min_step=-1, max_step=10000):
    # Import tensorflow lazily because it conflicts with torch CUDA init in
    # the same process (causes SIGSEGV during sim worker startup). Only the
    # ckpt-best lookup needs tf to read tensorboard event files.
