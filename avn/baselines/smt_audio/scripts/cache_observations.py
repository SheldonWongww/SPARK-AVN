...
def main(dataset):
    ...
    num_obs = 0
    scene_obs_dir = '/data1/wxy/code/ENMuS/data/scene_observations/' + dataset
    os.makedirs(scene_obs_dir, exist_ok=True)
    metadata_dir = '/data1/wxy/code/ENMuS/data/metadata/' + dataset
    for scene in os.listdir(metadata_dir):
        ...
        if dataset == 'replica':
            scene_id = os.path.join('/data1/wxy/data/replica_v1', dataset, scene, 'habitat/mesh_semantic.ply')
        else:
            scene_id = os.path.join('/data1/wxy/code/ENMuS/data/scene_datasets', dataset, scene, scene + '.glb')
