# cavn/model/state_encoder.py
import torch
import torch.nn as nn
import torch.nn.functional as F
from typing import Tuple, Optional

from cavn.model.transformer_decoder import MSMTDecoder, MSMTDecoderLayer
...
class MSMTStateEncoder(nn.Module):
    def __init__(
        ...
        if pose_indices is not None:
            pose_dims = pose_indices[1] - pose_indices[0] # number of pose dimensions
            self.pose_encoder = nn.Linear(5, 16)
            input_size += 16 - pose_dims # update input size
    ...
    def _format_p2t_masks(self, memory_masks):
        """ multi-scale memory masks for MSMT transformer """
        masks = [memory_masks] # [B, L], L = M + 1
        for i, ratio in enumerate(self._pool_ratios):
            if i == 0:
                ...
                masks.append(mask)
        masks = torch.cat(masks, dim=1) # [B, L_total], L_total = L + L/4 + L/8 + L/16 + L/32

        return masks

    def single_forward(self, x, memory, memory_masks, goal=None):
        # goal is belief, not used in current experiments
        ...
        if self._pretraining:
            memory_masks = torch.cat([torch.zeros_like(memory_masks), torch.ones([memory_masks.shape[0], 1], device=memory_masks.device)], dim=1)
        else:
            memory_masks = torch.cat([memory_masks, torch.ones([memory_masks.shape[0], 1], device=memory_masks.device)], dim=1)

        memory = torch.cat([memory, x.unsqueeze(0)]) # append current observation to memory,[M, bs, Input_dim]->[M+1, bs, Input_dim]
        M, bs = memory.shape[:2] # M=M+1
        memory = self.fusion_encoder(memory.view(M*bs, -1)).view(M, bs, -1) # [M, bs, Input_dim]->[M, bs, 256]

        t_masks = self._convert_masks_to_transformer_format(memory_masks)
        if "MSMT" in self.decoder_type.__name__:
            t_memory_masks = self._format_p2t_masks(memory_masks) # multi-scale masks for MSMT
            t_memory_masks = self._convert_masks_to_transformer_format(t_memory_masks)
        else:
            t_memory_masks = t_masks # standard transformer masks
        ...
        if goal is not None:
            ...
            if decode_memory:
                ...
                # Actually only enter this branch
                x_att = self.transformer(
                    memory,
                    memory[-1:], # query is the current observation
                    src_key_padding_mask=t_masks,
                    memory_key_padding_mask=t_memory_masks,
                    tgt_is_causal=False,
    ...
    def _encode_pose(self, agent_pose, memory_pose):
        """ Encode agent and memory poses into relative pose representations """
        ...
        memory_pose_encoded = self.pose_encoder(memory_pose_formatted.view(M * bs, -1)).view(M, bs, -1)

        return agent_pose_encoded, memory_pose_encoded

    def _compute_relative_pose(self, pose_a, pose_b):
        """ Compute relative pose of b w.r.t a """
        heading_a = -pose_a[..., 2]
        heading_b = -pose_b[..., 2]
        # Compute relative pose
        r_ab = torch.norm(pose_a[..., :2] - pose_b[..., :2], dim=-1) # distance
        phi_ab = torch.atan2(pose_b[..., 1] - pose_a[..., 1], pose_b[..., 0] - pose_a[..., 0]) # angle to b, global
        phi_ab = phi_ab - heading_a # angle to b, relative to a's heading
        x_ab = r_ab * torch.cos(phi_ab) # b's x in a's coord
        y_ab = r_ab * torch.sin(phi_ab) # b's y in a's coord
        ...
        heading_ab = -heading_ab # habitat y points downwards

        return torch.stack([x_ab, y_ab, heading_ab], -1) # (..., 3)

    def _format_pose(self, pose):
        """ Format pose into [x, y, cos(heading), sin(heading), exp(-time)] """
