# cavn/model/audio_encoder.py
import torch
import torch.nn as nn
import numpy as np

from cavn.common.utils import Flatten, MultiHeadAttentionLayer, conv_output_dim
...
class AudioCNN_GD(nn.Module):
    """CNN + Attention + MLP, No RNN involved."""
    def __init__(self, observation_space, output_size, audiogoal_sensor) -> None:
        super().__init__()
        self._number_input_audio = observation_space.spaces[audiogoal_sensor].shape[2] # channels
        self._audiogoal_sensor = audiogoal_sensor

        cnn_dimensions = np.array([65, 25]) # [freq_bins, time_frames]
    ...
    def forward(self, observations):
        ...
        audio_observations = audio_observations.permute(0, 3, 1, 2) # [B, C, F, T]
        audio_observations = audio_observations[:, :, :, -25:] #[B, C, F, 25]
