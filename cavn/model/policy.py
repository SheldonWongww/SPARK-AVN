# cavn/model/policy.py
import abc
import logging
import itertools
import torch
import torch.nn as nn
from soundspaces.tasks.nav import PoseSensor, LocationBelief, CategoryBelief

from cavn.env.nav import PoseSensorGD, SenCategory
from cavn.common.utils import CustomFixedCategorical
from cavn.model.visual_encoder import SMTCNN
from cavn.model.goal_descriptor import GoalDescriptor
from cavn.model.state_encoder import MSMTStateEncoder
from cavn.model.audio_encoder import AudioCNN_GD
...
class Policy(nn.Module):
    ...
    def act(self, observations, rnn_hidden_states, prev_actions, masks, ext_memory, ext_memory_masks, deterministic=False):
        # Get features from the encoder network
        ...
        if deterministic:
            action = distribution.mode() # Deterministic Policy
        else:
            action = distribution.sample() # Stochastic Policy
    ...
    def evaluate_actions(self, observations, rnn_hidden_states, prev_actions, masks, action, ext_memory, ext_memory_masks):
        # Get features from the encoder network
        features, rnn_hidden_states, ext_memory_feats = self.net(
...
class AudioNavMSMTNetWithGD(Net):
    def __init__(
        ...
        **kwargs
        ...
        # Audio Encoder
        self.goal_encoder = AudioCNN_GD(observation_space, 512, 'spectrogram')
        ...
        if self._use_goal_descriptor:
            ...
            self.goal_downsample = nn.Linear(256, 128) # GD Downsample to 128 dims
        ...
        if PoseSensor.cls_uuid in observation_space.spaces:
            ...
            if self._use_goal_descriptor:
                nfeats += 128 # Goal Descriptor feature dims
            pose_indices = (nfeats, nfeats + pose_dims)
            nfeats += pose_dims # Pose feature dims
        else:
            pose_indices = None
        ...
        if self._use_belief_encoder:
            # Belief Encoder is not used in the paper experiments
    ...
    def pretrained_initialization(self, audio_path, visual_path):
        """Load pretrained audio and visual encoders."""
        print("Loading pretrained audio and visual models...")
        audio_state_dict = torch.load(audio_path, map_location='cpu')['state_dict']
        visual_state_dict = torch.load(visual_path, map_location='cpu')['state_dict']
    ...
    def freeze_encoders(self):
        """Freeze audio, goal descriptor and visual encoders. Pose encoder is not frozen."""
        logging.info(f'AudioNavMSMTNet ===> Freezing audio, goal descriptor and visual encoders!')
        params_to_freeze = []
    ...
    def set_eval_encoders(self):
        """Sets the audio, goal descriptor and visual encoders to eval mode."""
    ...
    def get_features(self, observations, prev_actions):
        """Extract features from observations and previous actions."""
        x = []
        x.append(self.visual_encoder(observations)) # 128
        x.append(self.action_encoder(self._get_one_hot(prev_actions))) # 16
        x.append(self.downsample(self.goal_encoder(observations))) # 128
        if self._use_category_input:
            x.append(observations[SenCategory.cls_uuid]) # 20
        if self._use_goal_descriptor:
            x.append(self.goal_downsample(self.goal_descriptor(observations))) # 128
        x.append(observations[PoseSensor.cls_uuid]) # 4
        x = torch.cat(x, dim=1) # 424
        return x
    ...
    def forward(
        ...
        # state representation, rnn hidden states, external memory features
