# cavn/env/sound_eventnav_dataset.py
import gzip
import json
import os
import logging
...
from cavn.env.nav import ObjectViewLocation, SoundEventNavEpisode
...
from cavn.env.nav import SoundEventGoal

ALL_SCENES_MASK = "*"
CONTENT_SCENES_PATH_FIELD = "content_scenes_path"
DEFAULT_SCENE_PATH_PREFIX = os.path.join(
    os.environ.get("CAVN_DATA_ROOT", "data"), "scene_datasets"
) + os.sep
...
class SoundEventNavDataset(Dataset):
    ...
    def get_scenes_to_load(config: Config) -> List[str]:
        ...
        # If CONTENT_SCENES is explicitly set and not empty, use it directly
        if hasattr(config, 'CONTENT_SCENES') and len(config.CONTENT_SCENES) > 0:
            # Filter out wildcard '*' if present
            scenes = [s for s in config.CONTENT_SCENES if s != ALL_SCENES_MASK]
            if len(scenes) > 0:
                logging.info(f'Using CONTENT_SCENES from config: {scenes}')
                return scenes
        
        # Otherwise, get all available scenes from the dataset folder
        ...
        logging.info(f'Loading all available scenes from folder: {scenes}')
        return scenes
