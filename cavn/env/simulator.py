...
CAVN_DATA_ROOT = os.environ.get("CAVN_DATA_ROOT", "data")
...
class SoundEventNavSim(Simulator, ABC):
    ...
    def create_sim_config(
        ...
        sim_config.scene_dataset_config_file = os.path.join(
            CAVN_DATA_ROOT, "scene_datasets", "mp3d", "mp3d.scene_dataset_config.json"
        )
        ...
        if not self.config.USE_RENDERED_OBSERVATIONS:
            ...
            if not self.material_configured:
                audio_sensor.setAudioMaterialsJSON(
                    os.path.join(CAVN_DATA_ROOT, "mp3d_material_config.json")
                )
